...
func createIntegratePr(log *logrus.Entry, ghc githubClient, integratePrsRepo, topic string,
	...
	// Create Comment
	return ghc.CreateComment(ce.Repo.Owner.Login, ce.Repo.Name, ce.Number, fmt.Sprintf("Integrated with pr https://github.com/%s/pull/%d", integratePrsRepoInfo, number))
}
